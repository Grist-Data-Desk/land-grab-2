...
from land_grab_2.utilities.utils import GristCache, in_parallel
...
def find_overlaps(state, activity, activity_data, grist_data):
    try:
        ...
        grist_update_thus_far = capture_matches(matches, state, activity)
        return grist_update_thus_far
    except Exception as err:
        print(traceback.format_exc())
        print(f'failing on mysterious except in find_overlaps()')


def process_state_activity(grist_data, activity_state, activity_info, cache_dir, activity):
...
def match_all_activities(states_data=None, grist_data=None):
    ...
    for activity_state, activity_info in states_data.items():
        ...
        st = datetime.now()
        results = in_parallel(activity_info.activities,
                              partial(
                                  process_state_activity, grist_data, activity_state, activity_info, CACHE_DIR
                              ),
                              show_progress=True,
...
def main(stl_path: Path, the_out_dir: Path):
    ...
    for row_idx, activity_list in GRIST_DATA_UPDATE.items():
        gdf.loc[row_idx, ACTIVITY] = ','.join(list(sorted(list(activity_list))))
