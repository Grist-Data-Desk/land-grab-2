...
from land_grab_2.utilities.overlap import eval_overlap_keep_left, STATE_LONG_NAME
...
def extract_matches(state, activity, activity_data, grist_data, overlap_report):
    for name, group in overlap_report.groupby("joinidx_1")[["joinidx_0"]]:
        activity_row = activity_data[activity_data['joinidx_1'] == name]
        activity_name = get_activity_name(state, activity, activity_row)

        grist_row_idxs = group['joinidx_0'].tolist()
        grist_rows = grist_data[grist_data['joinidx_0'].isin(grist_row_idxs)].index.tolist()
...
def extract_matches_latest(state, activity, activity_data, grist_data, overlap_report):
    for name, group in overlap_report.groupby("joinidx_1")[["joinidx_0"]]:
        activity_row = activity_data[activity_data['joinidx_1'] == name]
        activity_name = get_activity_name(state, activity, activity_row)
...
def find_overlaps_latest(state, activity, activity_data, grist_data):
    try:
        crs_list = GristDB().crs_search_by_state(STATE_LONG_NAME[state])
        crs_list = [f"{r['data_source']}:{r['coord_ref_sys_code']}" for r in crs_list]
        crs_list = [grist_data.crs] + crs_list

        overlapping_regions, left, right = eval_overlap_keep_left(activity_data,
                                                                  grist_data,
                                                                  crs_list=crs_list,
                                                                  return_inputs=True)
