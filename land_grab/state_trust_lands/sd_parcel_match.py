...
# Now we compare the list of reformatted secdivids called from the server
  if sub_reformatted_secdivids in all_adjustedstls:
    ...
    done_csv_writer.writerow(secdivid_data['features'][0]['attributes'])
    all_adjustedstls.remove(sub_reformatted_secdivids)
    return

  if surf_reformatted_secdivids in all_adjustedstls:
    log.info(f'writing done-row for original: {secdivid} surf_reformatted_secdivids: {surf_reformatted_secdivids}')
    secdivid_data['features'][0]['attributes']['geometry'] = geo
    done_csv_writer.writerow(secdivid_data['features'][0]['attributes'])
    all_adjustedstls.remove(surf_reformatted_secdivids)
    return


def match_adjusted_sdstllist_with_sdqq_server(adjusted_surface,
                                              adjustedsurface_column_name,
                                              adjusted_subsurface,
...
# This function gets all the OBJECTIDS and creates the output files and starts the parallel/concurrent hydration
  ...
  all_adjustedstls = set(surface + subsurface)
  ...
  with open(Path('~/Desktop/unmatched.csv').expanduser().resolve(), 'w') as unmatched_csv_writer_fh:
    ...
    for row in all_adjustedstls:
      unmatched_csv_writer.writerow([row])


def main():
  log.info('importing subsurface list')
  subsurface_list = import_csv('/Users/mpr/Documents/01_Current Projects/Grist_LGU2/SD-REAL/0711_SUBSURFACE SIMPLE.csv')

  log.info('importing surface list')
  surface_list = import_csv('/Users/mpr/Documents/01_Current Projects/Grist_LGU2/SD-REAL/0711_SURFACE SIMPLE.csv')

  log.info('fix_sdstl_lists subsurface ')
  adjusted_subsurface = fix_sdstl_lists(subsurface_list, 10)
  adjusted_subsurface.to_csv(r'/Users/mpr/Desktop/sd_subsurface.csv')

  log.info('fix_sdstl_lists surface ')
  adjusted_surface = fix_sdstl_lists(surface_list, 7)
  adjusted_surface.to_csv(r'/Users/mpr/Desktop/sd_surface.csv')

  log.info('match_adjusted_sdstllist_with_sdqq_server')
  match_adjusted_sdstllist_with_sdqq_server(adjusted_surface,
                                            'FIXED_PLSS_NUMBER',
                                            adjusted_subsurface,
                                            'FIXED_PLSS_NUMBER',
                                            starting_record=152261)  #152261 - first objID that matched # 368987)
